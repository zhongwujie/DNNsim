...
def get_layer_list(model_folder: str) -> list:
  model_path = os.path.join(model_folder, "model.txt")
  layer_list = []
...
def gen_net_trace(model_folder: str):
  ...
  for layer_str in layer_list:
    ...
    act_path = os.path.join(net_trace_folder, "act_layer{}-0.npy".format(layer_id))
    wei_path = os.path.join(net_trace_folder, "weight_layer{}.npy".format(layer_id))
...
def test():
  get_model_folders("./models/benchmark")
