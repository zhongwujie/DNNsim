...
namespace core
{
																												...
																												const base::Array<T> &act, const base::Array<T> &wgt)
	{
        ...
        std::cout << "cycles: " << PE_cycles << std::endl;
				...
				// std::cout << "PE_cycles: " << PE_cycles << std::endl;
		...
		tile_stats.offchip_weight_reads += tile_max_i_loop * wgt_size; // only one PE ？
	...
	void SCNN<T>::run(const base::Network<T> &network)
	{
		...
		if (!QUIET)
			std::cout << "Starting cycles simulation for architecture SCNN" << std::endl;
